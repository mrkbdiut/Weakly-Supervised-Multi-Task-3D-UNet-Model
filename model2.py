...
class Modified3DUNet(nn.Module):
	def __init__(self, in_channels=1, n_classes=1, base_n_filter = 8):
		...
		self.dropout3d = nn.Dropout3d(p=0.01)
		...
		self.conv1 = nn.Conv3d(in_channels=64, out_channels=64, kernel_size=3, padding=1, stride=1, bias=False)
		# self.bn1 = nn.BatchNorm3d(in_channels)
		self.relu = nn.ReLU(inplace=True)
		# self.conv2 = conv3x3x3(in_channels, in_channels)
		# self.bn2 = nn.BatchNorm3d(in_channels)
		# self.downsample = downsample
		self.bn1=nn.BatchNorm3d(64, affine=False)
		self.avgpool = nn.AvgPool3d((int(4), int(4), int(4)), stride=1)
	...
	def forward(self, x):
		...
		seg_layer = out
		seg_layer = self.sigmoid(seg_layer)

		eps = nn.Parameter(torch.ones(1, ))
		nn.init.constant_(eps, 0.5)
		# x = torch.rand(15,1,32,32,32)
		s1=seg_layer
		# y = torch.zeros(50,1,32,32,32)
		# z = torch.ones(50,1,32,32,32)
		# out1 = torch.sigmoid((s1 - eps) * 100) * (z - y) + y
